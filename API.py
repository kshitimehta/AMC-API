...
def q_emissions_facility_boston():
    ...
    years = pd.read_sql("select distinct stay_year from emissions_by_zipcode", dbstring)
    years = years['stay_year'].astype(int).sort_values().tolist()
    facilities = pd.read_sql("select distinct building_name from emissions_by_zipcode", dbstring)
